...
import { Moon, Sun, Monitor, Minus, Plus, X, Footprints, AlertCircle, UserCircle2, GripVertical, Settings, LogOut } from "lucide-react";
      ...
      <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in md:p-4">
        <div className="bg-white dark:bg-stone-900 rounded-none md:rounded-3xl px-6 md:px-8 shadow-2xl w-full h-full md:h-auto md:max-h-[85vh] max-w-lg flex flex-col relative animate-in zoom-in-95 border-0 md:border border-stone-200 dark:border-stone-800 overflow-y-auto pt-[calc(2rem+env(safe-area-inset-top))] pb-[calc(3rem+env(safe-area-inset-bottom))] md:py-8 gap-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="mt-2 p-4 bg-stone-50 dark:bg-stone-950 border border-stone-100 dark:border-stone-800 rounded-xl">
            ...
            <>
              <div className="flex flex-col gap-3">
                ...
                >
                  ...
                  <span className="text-xs text-sky-600 dark:text-sky-400 font-bold">변경</span>
                </button>
              </div>
              ...
              {onLogout && (
                <button
                  onClick={onLogout}
                  className="flex items-center justify-center gap-2 w-full py-3.5 bg-stone-100 hover:bg-stone-200 dark:bg-stone-800 dark:hover:bg-stone-700 text-stone-700 dark:text-stone-300 font-bold rounded-xl transition-colors border border-stone-200 dark:border-stone-700"
                >
                  <LogOut size={18} />
                  로그아웃
                </button>
        ...
        {/* Reset Confirm Modal */}
        {isResetModalOpen && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in" onClick={() => setIsResetModalOpen(false)}>
