...
const formatDate = (value: string | null | undefined) => value ? new Date(value).toLocaleString("ko-KR", { dateStyle: "medium", timeStyle: "short" }) : "기록되지 않음";
const formatDay = (value: string | null | undefined) => value ? new Date(`${value}T00:00:00+09:00`).toLocaleDateString("ko-KR") : "-";
...
function AdminDashboardContent() {
  ...
  return (
    <div data-admin-dashboard className="min-w-0 bg-stone-50 px-4 pb-[calc(1.5rem+env(safe-area-inset-bottom))] pt-[calc(1.5rem+env(safe-area-inset-top))] text-stone-900 dark:bg-stone-950 dark:text-stone-100 sm:px-6">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <header className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 rounded-full bg-amber-100 px-3 py-1.5 text-xs font-bold text-amber-800 dark:bg-amber-900/30 dark:text-amber-300"><ShieldCheck size={16} /> 읽기 전용</div>
        </header>
        ...
        <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.35fr)]">
          ...
          <div className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-800 dark:bg-stone-900">
            {!detail ? <div className="flex min-h-72 items-center justify-center text-sm text-stone-500">사용자를 선택하면 상세 점검 내용이 표시됩니다.</div> : <UserDetailPanel detail={detail} selectedDay={selectedDay} setSelectedDay={setSelectedDay} notifications={notifications} onLoadNotifications={(day, filters) => void loadNotifications(detail.profile.id, day, filters)} onCopy={copyId} />}
          </div>
        </section>
      </div>
    </div>
...
function UserDetailPanel({ detail, selectedDay, setSelectedDay, notifications, onLoadNotifications, onCopy }: { detail: UserDetail; selectedDay: number | null; setSelectedDay: (day: number | null) => void; notifications: NotificationRow[]; onLoadNotifications: (day: number | null, filters?: NotificationFilters) => void; onCopy: (id: string) => void }) {
  ...
  return <div className="flex flex-col gap-5">
    ...
    <div><h3 className="mb-2 font-bold">Day별 기록</h3><div className="rounded-xl border border-stone-200 dark:border-stone-800">{detail.records.length === 0 ? <p className="p-4 text-sm text-stone-500">기록이 없습니다.</p> : detail.records.map((record) => <button key={record.dayIndex} onClick={() => { setSelectedDay(record.dayIndex); onLoadNotifications(record.dayIndex); }} className={`flex w-full flex-col gap-1 border-b border-stone-100 p-3 text-left text-xs last:border-0 dark:border-stone-800 ${record.dayIndex === selectedDay ? "bg-sky-50 dark:bg-sky-900/20" : "hover:bg-stone-50 dark:hover:bg-stone-800"}`}><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-black">Day {record.dayIndex} · {formatDay(record.readDate)}</span><span className={record.completedAt ? "text-emerald-600" : "text-stone-400"}>{record.completedAt ? `완료 ${formatDate(record.completedAt)}` : "미완료"}</span></div><div className="flex flex-wrap gap-2 text-stone-500"><span>{record.hasOneVerse ? `One Verse ${record.verseLocation?.book || ""} ${record.verseLocation?.chapter || ""}:${record.verseLocation?.verse || ""}` : "One Verse 없음"}</span><span>{record.isMemorized ? `마음새김 ${record.memorizedMethods.join(", ") || "완료"}` : "마음새김 없음"}</span><span>{record.hasFootprint ? "발자국 있음" : "발자국 없음"}</span></div></button>)}</div></div>
