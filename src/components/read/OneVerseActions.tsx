  ...
  return (
    <>
      <div className="mt-4 grid grid-cols-3 gap-2 border-t border-stone-200 pt-3 pl-[2ch] sm:pl-[2.5ch] dark:border-stone-800/50">
        <button onClick={(event) => { event.stopPropagation(); onOpenMemory(); }} className={`min-h-11 min-w-0 rounded-lg border text-xs font-bold flex items-center justify-center gap-1 transition-colors ${isMemorized ? "border-amber-200 bg-amber-50 text-amber-600 hover:bg-amber-100 dark:border-amber-800 dark:bg-amber-900/30 dark:text-amber-400" : "border-stone-200 bg-white text-stone-700 hover:bg-stone-50 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300 dark:hover:bg-stone-700"}`}>
          <Heart size={15} />마음새김
        </button>
        <button onClick={(event) => { event.stopPropagation(); router.push(`/memo?day=${dayIndex}&mode=${verse.memo ? "view" : "edit"}`); }} className="min-h-11 min-w-0 rounded-lg border border-stone-200 bg-white text-xs font-bold text-stone-700 flex items-center justify-center gap-1 transition-colors hover:bg-stone-50 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300 dark:hover:bg-stone-700">
          <Footprints size={15} />발자국
        </button>
        <button onClick={(event) => { event.stopPropagation(); if (record) onShare(record); }} className="min-h-11 min-w-0 rounded-lg border border-stone-200 bg-white text-xs font-bold text-stone-700 flex items-center justify-center gap-1 transition-colors hover:bg-stone-50 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300 dark:hover:bg-stone-700"><HeartHandshake size={15} />나눔</button>
      </div>
