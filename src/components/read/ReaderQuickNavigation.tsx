...
import { BookMarked, BookOpen, ChevronsDown, Highlighter, Lightbulb, Music2, Pin, type LucideIcon } from "lucide-react";
  ...
  return (
    <div className="fixed right-0 top-1/2 -translate-y-1/2 z-40 ios-pwa-bottom-safe mr-1 md:mr-2 pointer-events-auto select-none touch-manipulation">
      <div className="flex flex-col gap-1 md:gap-1.5">
        ...
        <button onClick={() => onNavigate("viewer-bottom")} aria-label="본문 맨 아래로 이동" className="flex flex-col items-center justify-center w-11 h-11 md:w-16 md:h-16 aspect-square rounded-full bg-stone-800/80 hover:bg-stone-700 active:scale-95 text-stone-300 border border-stone-700/50 shadow-md transition-all hover:scale-105 gap-0 md:gap-1 cursor-pointer" title="맨 아래">
          <ChevronsDown size={18} strokeWidth={2} className="pointer-events-none md:h-6 md:w-6" /><span className="text-[9px] md:text-xs font-medium whitespace-nowrap pointer-events-none">맨 아래</span>
        </button>
      </div>
    </div>
  );
