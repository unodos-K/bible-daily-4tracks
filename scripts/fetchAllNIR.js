...
async function fetchChapterWithRetry(bookCode, chapter, retries = 3) {
  // SAENEW is the actual version parameter for 새번역 on the Korean Bible Society
  const url = `https://www.bskorea.or.kr/bible/korbibReadpage.php?version=SAENEW&book=${bookCode}&chap=${chapter}`;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      $('span').each((_, el) => {
        const text = $(el).text();
        const numberSpan = $(el).find('span.number');
        if (numberSpan.length > 0) {
          const vNum = parseInt(numberSpan.text().trim().replace(/&nbsp;/g, ''));
          // Get the pure text without the number
          let verseText = $(el).clone();
          verseText.find('span.number').remove();
          verseText.find('a.comment').remove(); // remove footnote links
          verseText.find('div').remove(); // remove tooltips
          
          let cleanText = verseText.text().replace(/\n/g, '').replace(/\r/g, '').replace(/&nbsp;/g, ' ').trim();
          if (vNum && !isNaN(vNum)) {
            versesMap[vNum.toString()] = cleanText;
          }
        }
      });
      return versesMap;
...
async function main() {
  console.log(`Starting to fetch NIR (SAENEW) Bible from bskorea.or.kr...`);
  ...
  for (const book of BIBLE_BOOKS) {
    ...
    for (let chapter = 1; chapter <= book.chapters; chapter++) {
      try {
        const versesMap = await fetchChapterWithRetry(book.code, chapter);
        result[book.korean][chapter.toString()] = versesMap;
        totalChaptersFetched++;
        // 50ms delay as requested
        await new Promise(r => setTimeout(r, 50));
      } catch (err) {
        console.error(`Error fetching ${book.korean} ${chapter}:`, err.message);
      }
    }
    totalBooksFetched++;
  }
